...
class Requirement(BaseModel):
    ...
    def title_must_contain_only_characters_or_figure(cls,title:str) -> str:
        if has_punctuation_or_accent(title):
            raise ValueError(f"Title property '{title}' shall be composed of numeric" +
    ...
    class Config:
        ...
        extra = 'forbid' # unknow field is not permitted for the requirement class
...
class ReqFile(BaseModel):
    ...
    def validate_extension(cls, value):
        ...
        Raises:
            ValueError: If the file path does not have an appropriate extension.
        """
        path = Path(value)
        file_extension = path.suffix.lower()
        if file_extension != "" and file_extension in RequirementFileSettings.allowed_extensions:
            return path
        else:
            raise ValueError(
                f"The filepath {value} does not have an appropriate extension, i.e [{RequirementFileSettings.allowed_extensions}]"
            )
    ...
    def write(self, requirement):
        ...
        Notes:
            ...
            - This method uses pydantic_encoder to convert the Requirement object to JSON.
        """
